...
lukkariControllers.controller('TodayCtrl', ['$scope', 'Timetables', '$ionicLoading', 'LocalStorage', '$ionicModal', 'MyDate', function ($scope, Timetables, $ionicLoading, LocalStorage, $ionicModal, MyDate) {
    $scope.groupInfo = {};
...
lukkariControllers.controller('AppointmentCtrl', ['$scope', 'Timetables', '$ionicLoading', '$stateParams', function ($scope, Timetables, $ionicLoading, $stateParams) {
    $scope.appointment = Timetables.getAppointment($stateParams.id);
}]);

// controller for weekly view
lukkariControllers.controller('WeekCtrl', ['$scope', 'Timetables', '$ionicLoading', '$ionicModal', 'LocalStorage', 'MyDate', 'Lessons', function ($scope, Timetables, $ionicLoading, $ionicModal, LocalStorage, MyDate, Lessons) {
    $scope.groupInfo = {};
    $scope.week = {};
    $scope.weekOffset = 0;
    $scope.groupInfo.group = LocalStorage.get('groupName');
...
lukkariControllers.controller('SettingsCtrl', ['$scope', 'LocalStorage', '$cordovaToast', '$ionicPlatform', '$cookies', '$timeout', '$cordovaCalendar', 'Timetables', function ($scope, LocalStorage, $cordovaToast, $ionicPlatform, $cookies, $timeout, $cordovaCalendar, Timetables) {
    $scope.groupInfo = {};
    $scope.reminder = {};
...
lukkariServices.factory('Lessons', ['$http', function ($http) {
    var lessons = [];
    var savedGroupName = '';
...
lukkariServices.factory('Timetables', ['$http', 'ical', '$cookies', 'ApiEndpoint', 'MyDate', function ($http, ical, $cookies, ApiEndpoint, MyDate) {
    var appointments = [];
    ...
    function toICAL() {
        ...
        for (var i = 0; i < appointments.length; i++) {
            ...
            // set properties
            event.summary = appointments[i].summary + ' ' + appointments[i].courseNumber;
            event.status = 'ACCEPTED';
            event.organizer = appointments[i].teacher;
            event.location = appointments[i].location;
            vEvent.addPropertyWithValue('dtstart', ical.Time.fromJSDate(appointments[i].startDate));
            vEvent.addPropertyWithValue('dtend', ical.Time.fromJSDate(appointments[i].endDate));
    ...
    function makeRequest(groupName, startDate, endDate, callback) {
        ...
        $http({
            ...
            $http({
                method: 'GET',
                url: ApiEndpoint.url + '/icalcreator.php?startDate=' + MyDate.formatDay(startDate, true) + '&endDate=' + MyDate.formatDay(endDate, true)
            }).then(function (response) {
    ...
    function parseEvent(vEvent, index) {
        ...
        try {
            appointment.locationInfo = event.location.slice(appointment.location.length + 2).split(', ')[0];
            appointment.locationInfo2 = event.location.slice(appointment.location.length + 2).split(', ')[1];
        } catch (e) {
        ...
        try {
            appointment.teacher = event.description.split(/Henkilö\(t\): /)[1].split(/Ryhmä\(t\): /)[0];
        ...
        try {
            appointment.groups = event.description.slice(event.description.split(/Ryhmä\(t\): /)[0].length).split(/Ryhmä\(t\): /)[1];
        } catch (e) {
