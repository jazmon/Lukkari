...
angular.module('lukkari.services').factory('FoodService', ['$http', 'LunchEndPoint', 'ngXml2json', function ($http, LunchEndPoint, ngXml2json) {
  ...
  function parseLunch(element, index, array) {
    ...
    lunch.dish = element.div[1].div[0].div.div.ul.li.div.div.div[0].div.div.content;
    console.log(lunch.dish);
    lunch.dishes = [];
    // remove 3 from length to ignore evening foods
    var length = element.div[1].div.length - 3;
    for (var i = 0; i < length; i++) {
      var dish = element.div[1].div[i].div.div.ul.li.div.div.div[0].div.div.content;
...
angular.module('lukkari.controllers').controller('LunchCtrl', ['$scope', 'FoodService', 'ionicMaterialInk', 'ionicMaterialMotion', function ($scope, FoodService, ionicMaterialInk, ionicMaterialMotion) {
  FoodService.get({
